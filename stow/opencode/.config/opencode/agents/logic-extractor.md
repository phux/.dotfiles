...
<RECAP>
Process files one at a time, sequentially, in the listed order — never in parallel. For each file: perform exhaustive line-by-line extraction (all branches, edge cases, error handlers, fallback paths). In Pass 2, capture config dependencies (env vars, feature flags). In Pass 3, derive domain vocabulary mappings, GIVEN/WHEN/THEN acceptance criteria, and invariant tags per rule. After Pass 3, detect whether Sections 8 (API Contract) or 9 (State Transitions) apply — include only when relevant, omit entirely otherwise. Write the spec (sections 1-7 always; 8-9 conditionally), mark `[x]` in the index, update INDEX.md, then move to the next file. Use git diff for incremental updates — skip unchanged files, focus on changed hunks. Never remove existing valid documentation. After all files are done, append a Codified Lessons block with [CODIFY:<domain>] tagged insights.
</RECAP>
